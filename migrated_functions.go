...
func (fh *FunctionHandler) Uncapitalize(str string, delimiters string) string {
...
func (fh *FunctionHandler) WordWrap(str string, wrapLength int, newLineCharacter string, wrapLongWords bool) string {
	if wrapLength < 1 {
		wrapLength = 1
	}
	if newLineCharacter == "" {
...
func (fh *FunctionHandler) Indent(spaces int, v string) string {
	var builder strings.Builder
...
func (fh *FunctionHandler) Nindent(spaces int, v string) string {
	return "\n" + fh.Indent(spaces, v)
}
...
func (fh *FunctionHandler) Trunc(c int, s string) string {
	length := len(s)
...
func (fh *FunctionHandler) Substring(start, end int, s string) string {
	if start < 0 {
		start = len(s) + start
	}
	if end < 0 {
...
func (fh *FunctionHandler) Ceil(a any) float64 {
	aa := fh.ToFloat64(a)
	return math.Ceil(aa)
}

func (fh *FunctionHandler) Round(a any, p int, rOpt ...float64) float64 {
	roundOn := .5
	if len(rOpt) > 0 {
		roundOn = rOpt[0]
	}
	val := fh.ToFloat64(a)
	places := fh.ToFloat64(p)
...
// caseStyle dictates the transformation rules for the string.
type caseStyle struct {
	...
	InsertSeparator bool // Whether to insert a separator before each capital rune.
}

var (
	camelCaseStyle    = caseStyle{Separator: -1, CapitalizeNext: true, ForceLowercase: true, InsertSeparator: false}
	kebabCaseStyle    = caseStyle{Separator: '-', CapitalizeNext: false, ForceLowercase: true, InsertSeparator: true}
	pascalCaseStyle   = caseStyle{Separator: -1, CapitalizeNext: true, ForceLowercase: false, InsertSeparator: false}
	snakeCaseStyle    = caseStyle{Separator: '_', CapitalizeNext: false, ForceLowercase: true, InsertSeparator: true}
	sentenceCaseStyle = caseStyle{Separator: ' ', CapitalizeNext: true, ForceLowercase: true, InsertSeparator: true}
	dotCaseStyle      = caseStyle{Separator: '.', CapitalizeNext: false, ForceLowercase: true, InsertSeparator: true}
	pathCaseStyle     = caseStyle{Separator: '/', CapitalizeNext: false, ForceLowercase: true, InsertSeparator: true}
	constantCaseStyle = caseStyle{Separator: '_', CapitalizeNext: false, ForceLowercase: false, InsertSeparator: true}
)

func (fh *FunctionHandler) TransformString(style caseStyle, s string) string {
	var result strings.Builder
	result.Grow(len(s) + 10) // Allocate a bit more for potential separators
...
func (fh *FunctionHandler) ToCamelCase(s string) string {
	return fh.TransformString(camelCaseStyle, s)
}

func (fh *FunctionHandler) ToKebabCase(s string) string {
	return fh.TransformString(kebabCaseStyle, s)
}

func (fh *FunctionHandler) ToPascalCase(s string) string {
	return fh.TransformString(pascalCaseStyle, s)
}

func (fh *FunctionHandler) ToSentenceCase(s string) string {
	return fh.TransformString(sentenceCaseStyle, s)
}

func (fh *FunctionHandler) ToDotCase(s string) string {
	return fh.TransformString(dotCaseStyle, s)
}

func (fh *FunctionHandler) ToPathCase(s string) string {
	return fh.TransformString(pathCaseStyle, s)
}

func (fh *FunctionHandler) ToConstantCase(s string) string {
	return fh.TransformString(constantCaseStyle, s)
}

func (fh *FunctionHandler) ToSnakeCase(s string) string {
	return fh.TransformString(snakeCaseStyle, s)
}
...
func (fh *FunctionHandler) Add(values ...any) any {
	...
	reflectValue := reflect.ValueOf(values[0])
	result := float64(0)
	for _, value := range values {
		result += fh.ToFloat64(value)
	}

	// Convert the result to the same type as the input
	return reflect.ValueOf(result).Convert(reflectValue.Type()).Interface()

}

func (fh *FunctionHandler) Add1(x any) any {
	reflectX := reflect.ValueOf(x)
	one := reflect.ValueOf(1).Convert(reflectX.Type()).Interface()
...
func (fh *FunctionHandler) MulInt(values ...any) int64 {
	return fh.ToInt64(fh.Mulf(values...))
}
...
func (fh *FunctionHandler) Mulf(values ...any) any {
	...
	// Convert the result to the same type as the input
	return reflect.ValueOf(result).Convert(reflect.TypeOf(result)).Interface()
}
...
func (fh *FunctionHandler) Mod(x, y any) any {
	reflectX := reflect.ValueOf(x)
	result := math.Mod(fh.ToFloat64(x), fh.ToFloat64(y))
